...
npm run scrape                 # all 5 sites, all 4 categories, writes to DB
...
- `CATEGORIES` (`lib/types.ts`) is the single source of truth for the 4 tracked category slugs/labels; don't hardcode category lists elsewhere (navbar, sitemaps, filters all derive from it).
